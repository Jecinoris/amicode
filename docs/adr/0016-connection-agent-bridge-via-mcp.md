...
Connections bridge to agent-usable operations through MCP tools in the fork server (ADR 0002) — the same process that owns the credential store — rather than through standalone CLI binaries that read credential files from disk independently. The Slack integration is the first instance; the pattern extends to any Connection that needs an agent-facing API.
...
**Implementation:** see issue "Slack MCP tools: bridge the Connections panel to agent-usable Slack operations."
