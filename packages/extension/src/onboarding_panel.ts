 ...
 *  falling back to ~/.config/opencode/opencode.json. Matches opencodeDataDir's
 *  auth.json (below) and amicode-cli's harmoniqsPaths — all three must agree
 *  on where opencode.json/auth.json live, or a provider written by one host
 *  silently goes unseen by the other. */
