...
// The overlay IS the source of truth for the engine — edit it directly.
// The fork is archived; there is no external repo to push to.
// The ROOT AGENTS.md (the dev-agent instruction set) must teach this so
// every agent edits the right place.
describe("AGENTS.md overlay-as-source-of-truth", () => {
  const ROOT_AGENTS = readFileSync(join(__dirname, "..", "..", "..", "AGENTS.md"), "utf8");

  it("names the overlay as the source of truth", () => {
    expect(ROOT_AGENTS).toMatch(/overlay.*IS the source of truth/i);
  });

  it("names the overlay path for engine edits", () => {
    expect(ROOT_AGENTS).toMatch(/packages\/app-bundle\/overlay/);
  });

  it("teaches build:binary as the compile step", () => {
    expect(ROOT_AGENTS).toMatch(/build:binary/);
  });
});
