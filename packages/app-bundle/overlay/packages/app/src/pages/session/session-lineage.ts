...
export function createSessionLineage<T>(sessionID: () => string, lineage: () => LineageStore<T>) {
  ...
  createEffect(
    on([sessionID, lineage] as const, ([id, store]) => {
      ...
      import("../session").then((m) => m.holdDebug("lineage-pending", { id }))
      store
        .resolve(id)
        .then(() => {
          if (!stale) setStatus({ id, store, state: "settled" })
          import("../session").then((m) =>
            m.holdDebug("lineage-resolved", { id, peeked: !!store.peek(id) }),
          )
        })
        .catch((failure) => {
          if (!stale) setStatus({ id, store, state: "failed", failure })
          import("../session").then((m) => m.holdDebug("lineage-failed", { id, failure: String(failure).slice(0, 120) }))
        })
    }),
  )
